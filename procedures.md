...
- sda 및 파티션
  - SCSI Drive [a-z] = sda (물리)
    - sda1, sda2, ... = 파티션
  - 왜 부트로더를 /dev/sda에 설치해도 /dev/sda1에 설치되는가
...
- **(시험 필요)** root에게도 적용되도록 하고 싶은 규칙은 `enforce_for_root`
- **(시험 필요)** 사용자에게만 적용되도록 하고 싶은 규칙은 `local_users_only`
- 이전 암호와 중복되지 않는 글자 최소 7개: `difok=7`

```
minlen=10 ucredit=-1 dcredit=-1 lcredit=0 ocredit=0 maxrepeat=3 reject_username enforce_for_root difok=7
```
...
> 
> 이때 크레딧을 음수로 설정하면 가산점은 없어지고 해당 군의 문자를 N개 사용하는 것이 강제된다.
...
1. `donghyle` 사용자를 `user42`와 `sudo` 그룹에 추가한다.
...
> `ssh` 기능을 확인 시 VirtualBox의 포드포워딩 기능을 활용한다.
...
DISK_TOTAL=${echo "${DISK_USED}+${DISK_AVAIL}" | bc | tr -d '\n'}
...
N_SUDO=${journalctl _COMM=sudo | wc -l | tr -d '\n'}
...
1. 가상 기기의 상태 저장
1. 가상 기기의 스냅샷 찍기
1. `~/VirtualBox VMs/shasum $(가상 기기 이름).vdi` 실행하여 시그니쳐 확보
